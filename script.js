...
function loadStopwatchFromLocalStorage() {
  ...
  tenMilliseconds = parsed.time || 0;
  laps = parsed.laps || [];
  lapSecond = parsed.lapSecond || [];
...
function renderLaps() {
  ...
  for (let i = 0; i < laps.length; i++) {
    ...
    lap.textContent = "Lap " + lapNumber + " - " + laps[i] + " (" + getLapTime(i) +")";
    lapsContainer.appendChild(lap);
    lapNumber++;
  }
}

function getLapTime(i) {
  let oneLapSecond = lapSecond[i+1] - lapSecond[i];

  let lapHrs = Math.floor(oneLapSecond / 360000);
  let lapMins = Math.floor((oneLapSecond % 360000) / 6000);
  let lapSecs = Math.floor((oneLapSecond % 6000) / 100)
  let lapMils = oneLapSecond % 100;

  let oneLapTime =
    String(lapHrs).padStart(2, "0") + ":" +
    String(lapMins).padStart(2, "0") + ":" +
    String(lapSecs).padStart(2, "0") + "." +
    String(lapMils).padStart(2, "0");
...
function updateClock() {
  ...
  // first waxing moon day - 1 of the year
  const firstWax = -12;
...
const markBtn = document.querySelector(".mark-stopwatch");
...
function updateDisplay() {
  let hrs = Math.floor(tenMilliseconds / 360000);
  let mins = Math.floor((tenMilliseconds % 360000) / 6000);
  let secs = Math.floor((tenMilliseconds % 6000) / 100)
  let mils = tenMilliseconds % 100;

  display.textContent =
    String(hrs).padStart(2, "0") + ":" +
    String(mins).padStart(2, "0") + ":" +
    String(secs).padStart(2, "0") + "." +
    String(mils).padStart(2, "0");
}
...
markBtn.addEventListener("click", function() {
  // mark only when running
  if (interval === null) return;

  const lap = document.createElement("div");

  laps.push(display.textContent);
  lapSecond.push(tenMilliseconds);
  renderLaps();
  saveStopwatchToLocalStorage();

  console.log(lapSecond);
});

function saveStopwatchToLocalStorage() {
  const stopwatchState = {
    time: tenMilliseconds,
    laps: laps,
    lapSecond: lapSecond
  };

  localStorage.setItem("stopwatch", JSON.stringify(stopwatchState));
}

loadStopwatchFromLocalStorage();